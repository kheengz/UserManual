...
function validateImageFile(id, validationSize, btn) {
    id.bind('change', function () {
        var size = this.files[0].size;
        var value = $(this).val().toLowerCase();
        var extension = value.substring(value.lastIndexOf('.'));
        if ($.inArray(extension, ['.gif', '.png', '.jpg', '.jpeg']) === -1) {
            $('#image_error').html('<div class="alert alert-danger">\n\
            Invalid File Type. Require Only Image Files With Extensions of (.gif, .png, .jpg, .jpeg)</div>');
            btn.attr('disabled', 'disabled');
        } else if (size >= validationSize) {
            $('#image_error').html('<div class="alert alert-danger">\n\
            File Size To Large. Requires Only Files Less Than ' + (validationSize / 1024) + ' KB</div>');
            btn.attr('disabled', 'disabled');
        } else {
            $('#image_error').html('');
